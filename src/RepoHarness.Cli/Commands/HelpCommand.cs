...
internal static class HelpCommand
{
    ...
    private static string RenderRunners()
    {
        ...
        builder.AppendLine("declared before it; a step every run runs cannot need a manual one; and a predefined");
        builder.AppendLine("action cannot be manual - a manual step that must have one first names it under");
        builder.AppendLine("needs. A step's own inputs are its alone: another step naming one names nothing, and");
        builder.AppendLine($"a run that does not run the step refuses {CommandLineInputs.Option} for them. A {StepSelection.Option} naming a");
        builder.AppendLine("step the action lacks or one that is not manual, and a runner naming a step its action");
        builder.AppendLine("lacks, are refused before a host is measured, naming the steps there are.");
        builder.AppendLine();
