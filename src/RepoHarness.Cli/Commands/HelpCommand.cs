...
internal static class HelpCommand
{
    ...
    private static string RenderTools()
    {
        ...
        builder.AppendLine("something in it; declared empty, the built-in list stays. Each entry is absolute, or");
        builder.AppendLine("'~/' for the home directory of whoever searches. The harness's own .NET SDK is always");
        builder.AppendLine("looked for in the built-in list, so a repository's list cannot hide it.");
        builder.AppendLine();
        builder.AppendLine("The search runs on the machine that runs the leg, with one function for everything");
        builder.AppendLine("that asks: 'legs', the leg's own run, and install-missing-tools. A directory where a");
        builder.AppendLine("program was found off the PATH is appended to the PATH of every process the leg");
        builder.AppendLine("starts, so a build tool's own children - ninja and the compilers cmake starts - are");
        builder.AppendLine("found where it was. Appended, never prepended: a name the PATH already answers for");
        builder.AppendLine("keeps that answer.");
        builder.AppendLine();
        builder.AppendLine("'legs' refuses a leg whose host lacks a program its build or test starts: the");
        builder.AppendLine("project's build program, ninja for a Ninja generator, the compilers CC and CXX name,");
        builder.AppendLine("and the test runner. Not every declared tool - a tool one runner needs does not make");
        builder.AppendLine("every other leg on a host without it unrunnable. Such a leg, or one that still meets");
        builder.AppendLine("a program that will not start, is 'skipped-tool-missing': the run is incomplete,");
        builder.AppendLine("exit 21, never 'poisoned', which is kept for a defect in this tool.");
