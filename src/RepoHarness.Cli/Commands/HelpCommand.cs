...
internal static class HelpCommand
{
    ...
    private static string RenderTools()
    {
        ...
        builder.AppendLine("machine. A tool no selected leg on a host needs is not looked for there at all. A");
        builder.AppendLine("scope naming nothing declared, or one covering no declared leg, is refused.");
        builder.AppendLine();
    ...
    private static string RenderLegs()
    {
        ...
        builder.AppendLine("the C++ build tools by default - and a host that cannot turns the leg away as a");
        builder.AppendLine("tool missing, saying why. On the host that runs the leg, the instance its survey");
        builder.AppendLine("found has its vcvarsall.bat run for the leg's processor, cross-compiling where the");
        builder.AppendLine("host's differs, and what it set - cl, link and the Windows SDK on PATH, INCLUDE,");
        builder.AppendLine("LIB - is what every process of the leg starts with, so cl builds from a plain");
        builder.AppendLine("shell. One that fails, reports an [ERROR, or sets up another processor skips the");
        builder.AppendLine("leg before anything of it starts. Each leg's line names it: 'developer environment:");
        builder.AppendLine("visualStudio (Visual Studio 18.0.11205.157, MSVC 14.50.35717, amd64)', and");
        builder.AppendLine("developerEnvironment in --json. A copy starts nothing there, and needs none.");
        builder.AppendLine();
        ...
        builder.AppendLine("Only an object built under deps = msvc whose source includes nothing records none");
        builder.AppendLine("legitimately, and is excused: MSVC reports headers, never the source. How each");
        builder.AppendLine("object is built is read as ninja reads it, across the files build.ninja includes -");
        builder.AppendLine("CMake keeps its rules in CMakeFiles/rules.ninja - with its escapes undone.");
    ...
    private static string RenderAnchors()
    {
        ...
        builder.AppendLine("A citation a wrapped line cut is reported as cut, whatever rows exist: one running");
        builder.AppendLine("into a hyphen that ends its line, or ending its line where the next opens with the");
        builder.AppendLine("hyphen that carries it on; one cut before it has two segments counts when the next");
        builder.AppendLine("line carries on with what makes it a citation. A line ending with a citation and");
        builder.AppendLine("followed by one opening with a hyphen and a word, such as the option -O2, reads as");
        builder.AppendLine("a cut too. A break inside a segment, with no hyphen on either side, cannot be told");
        builder.AppendLine("from a line that simply ends there: it reads as the shorter id, reported unresolved");
        builder.AppendLine("unless that shorter id is a row of its own. Keep every id whole on one line.");
        builder.AppendLine();
    ...
    private static string RenderLayout()
    {
        ...
        builder.AppendLine("runner/.env directory, and git re-includes no placeholder from an excluded one.");
        builder.AppendLine();
        ...
        builder.AppendLine(".gitignore and its placeholders. The main checkout is left as it is.");
        builder.AppendLine();
